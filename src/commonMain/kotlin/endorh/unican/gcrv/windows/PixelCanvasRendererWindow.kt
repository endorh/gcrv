...
class PixelCanvasRendererWindow(scene: LineAlgorithmsScene) : BaseWindow("Conway`s Game of Life", scene), CoroutineScope {
    ...
    private inner class PixelCanvasRenderer : Composable {
                ...
                coroutineScope {
                    for (i in 0..<parallelism) launch {
                        ...
                        for (y in ys * i..<if (i == parallelism - 1) h else ys * (i + 1)) {
                            ensureActive()
                            for (x in 0..<w)
                                c[x, y] = cellColor(gameState, aliveness, x, y)
                        }
                    }
                }
                val canvas = canvas
        ...
        fun cellColor(gameState: ByteArray2D, aliveness: FloatArray, x: Int, y: Int): Int =
            if (gameState[x, y] > 0) lastColors.primary.RGBA.I
