...
@Serializable @SerialName("hilbert-curve")
object HilbertCurveRenderer : RecursiveGeoFractalRenderer {
   private val SQRT_3 = sqrt(3F)
   private val SQRT_3_6 = SQRT_3 / 6F

   override val displayName get() = "Hilbert Curve"
   ...
   override fun filter(geometry: Object2D, step: Int, subStep: Int, viewport: Rect2f): Boolean {
      ...
      val a: Vec2f
      val b: Vec2f
      if (geometry is PolyLineObject2D) {
         a = geometry.points.entries[0].value
         b = geometry.points.entries[1].value
      } else {
         geometry as LineObject2D
         a = geometry.start
         b = geometry.end
      }
      val r = abs(a.x - b.x + a.y - b.y)
      val cc = viewport.clamp(c)
      val d = abs(cc.x - c.x + cc.y - c.y)
      return d < r * 2F
   }
}